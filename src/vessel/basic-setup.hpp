 ...
 ** packager.
 ** 
 ** @see vessel::Voyage::Voyage()
 ** @see main.cpp
 ** 
 */


#ifndef VESSEL_BASIC_SETUP_H
#define VESSEL_BASIC_SETUP_H
      ...
      __ensure_hasKey (string const& key)  const
        {
          if (!util::contains (settings, key))
            throw err::Logic ("Key \""+key+"\" not found in setup.ini");
        }
    };
  
  
} // namespace vessel
#endif /*VESSEL_BASIC_SETUP_H*/
