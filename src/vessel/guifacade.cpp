...
namespace stage {
  ...
  struct GuiRunner
    : util::NonCopyable
    {
      using GuiHandle = InstanceHandle<LUMIERA_INTERFACE_INAME(lumieraorg_Gui, 1)>;
  ...
  namespace { // implementation of GUI-"Subsystem" : start GUI through GuiStarterPlugin
    ...
    class GuiSubsysDescriptor
      : public vessel::Subsys,
        public Sync<RecursiveLock_NoWait>
      {
        operator string()  const { return "Lumiera GTK GUI"; }
        
        bool
        shouldStart (vessel::Option& opts)  override
          {
        ...
        bool
        start (vessel::Option&, Subsys::SigTerm termNotification)  override
          {
            Lock guard{this};
            if (facade)
