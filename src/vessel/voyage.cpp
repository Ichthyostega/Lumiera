/*
  Voyage  -  global application initialisation and behaviour
 ...
 ** The vessel::Voyage object is set off by the `main()` function
 ** and coordinates the lifecycle of all parts of the application.
 ** 
 ** @see main.cpp
 */
...
#include "vessel/subsystem-runner.hpp"
...
namespace vessel {
  ...
  /** storage for the Voyage Singleton instance */
  lib::Depend<Voyage> Voyage::access;
  
  
  string
  Voyage::fetchSetupValue (Literal key)
  {
...
#define _MAYBE_THROW_ \
  ...
  void
  Voyage::maybeStart (Subsys& subsys)
  {
  ...
  using ExitCode = Voyage::ExitCode;
  ...
  ExitCode
  Voyage::abort (lumiera::Error& problem)
  {
   ...
   *  the normal shutdown sequence can be placed into the Voyage dtor.
